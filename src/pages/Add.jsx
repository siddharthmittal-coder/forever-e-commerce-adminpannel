  ...
  return (
    <form onSubmit={handleSubmit} className='flex flex-col items-start gap-3 w-full'>
      ...
      <div className='w-full'>
        ...
        <textarea onChange={(e) =>setDescription(e.target.value)} value={description} className='w-full max-w-125 px-3 py-2' type="text" placeholder='Write content here' />
      </div>
      ...
      <div>
        ...
        <div className='flex gap-3'>
          <div onClick={() => setSize(prev => prev.includes("S")? prev.filter(item => item !== "S"):[...prev,"S"])}>
            <p className={`${size.includes("S")? "bg-pink-100 border-2 text-black border-e-black" :"bg-slate-200 text-black"} px-2 py-1 cursor-pointer `}>S</p>
          </div>
          <div onClick={() => setSize(prev => prev.includes("M")? prev.filter(item => item !== "M"):[...prev,"M"])}>
            <p className={`${size.includes("M")? "bg-pink-100 border-2 text-black border-e-black" :"bg-slate-200 text-black"} px-2 py-1 cursor-pointer `}>M</p>
          </div>
          <div onClick={() => setSize(prev => prev.includes("L")? prev.filter(item => item !== "L"):[...prev,"L"])}>
            <p className={`${size.includes("L")? "bg-pink-100 border-2 text-black border-e-black" :"bg-slate-200 text-black"} px-2 py-1 cursor-pointer `}>L</p>
          </div>
          <div onClick={() => setSize(prev => prev.includes("XL")? prev.filter(item => item !== "XL"):[...prev,"XL"])}>
            <p className={`${size.includes("XL")? "bg-pink-100 border-2 text-black border-e-black" :"bg-slate-200 text-black"} px-2 py-1 cursor-pointer `}>XL</p>
          </div>
          <div onClick={() => setSize(prev => prev.includes("XXL")? prev.filter(item => item !== "XXL"):[...prev,"XXL"])}>
            <p className={`${size.includes("XXL")? "bg-pink-100 border-2 text-black border-e-black" :"bg-slate-200 text-black"} px-2 py-1 cursor-pointer `}>XXL</p>
          </div>
        </div>
      </div>
      ...
      <button className='mt-3 bg-black text-white  py-3 w-28 cursor-pointer hover:bg-gray-600 '>ADD</button>
    </form>
  )
}
